...
import { purchaseOrderTotals, vendorDisplayName } from "./purchase-order";
...
export function PurchaseOrderPdf({ draft, settings }: PurchaseOrderPdfProps) {
  ...
  const shipToLines = addressLines(settings.address);
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={styles.titleBlock}>
            <Text style={[styles.title, { color: accent }]}>Purchase order</Text>
            <Text style={styles.titleNumber}>{draft.poNumber}</Text>
          </View>
        </View>
        ...
        <View style={styles.parties}>
          <View style={styles.partyBlock}>
            ...
            <Text style={styles.bold}>{vendorDisplayName(draft.vendor)}</Text>
            {draft.vendor.name.trim() !== "" && draft.vendor.company.trim() !== "" && (
              <Text>{draft.vendor.company}</Text>
            )}
            {addressLines(draft.vendor.address).map((line, i) => (
              <Text key={i}>{line}</Text>
            ))}
            {draft.vendor.email.trim() !== "" && (
              <Text style={styles.muted}>{draft.vendor.email}</Text>
            )}
            {draft.vendor.phone.trim() !== "" && (
              <Text style={styles.muted}>{draft.vendor.phone}</Text>
            )}
          </View>
          {shipToLines.length > 0 && (
            <View style={styles.partyBlock}>
              <Text style={styles.sectionLabel}>Ship to</Text>
              {settings.businessName.trim() !== "" && <Text>{settings.businessName}</Text>}
          ...
          <View>
            <MetaRow label="Order date" value={formatIsoDate(draft.orderDate)} />
            {draft.expectedDate !== null && (
              <MetaRow label="Expected" value={formatIsoDate(draft.expectedDate)} />
            )}
          </View>
        </View>
...
export async function purchaseOrderPdfBlob(
  draft: PurchaseOrderDraft,
  settings: BusinessSettings,
): Promise<Blob> {
  return pdf(<PurchaseOrderPdf draft={draft} settings={settings} />).toBlob();
}
